...
#include "../third-party/glfw/include/GLFW/glfw3.h"
 ...
 Provider::Provider() {
  ...
  fs::path Provider::getConfigDir(fs::path& base) {
     fs::path configDir = base / ".config" / "ledit";
     if(fs::exists(configDir))
      return configDir;
    return base / ".ledit";
  }
    std::string Provider::getConfigPath() {
    fs::path *homeDir = getHomeFolder();
    if (!homeDir)
      return "";
    ...
    bool Provider::loadTheme(std::string name) {
  ...
  int Provider::runCommand(std::string command) {
    if (command_running)
      return 0;
   ...
   std::pair<int, std::string> Provider::runDirect(std::string command,
                                        std::string folder) {
#ifdef _WIN32
    HANDLE hStdoutRead, hStdoutWrite;
    SECURITY_ATTRIBUTES saAttr;
  ...
  std::string Provider::getBranchName(std::string path) {
    ...
    int32_t Provider::getNumberOrDefault(json o, const std::string entry, int32_t def) {
    if (!o.contains(entry))
      return def;
    json e = o[entry];
    if (!e.is_number())
    ...
    std::string Provider::getStringOrDefault(json o, const std::string entry,
                                 std::string def) {
    if (!o.contains(entry))
      return def;
  ...
  const fs::path Provider::getDefaultFontDir() {
// no idea how to do this differently
  ...
  void Provider::setVimRemaps(json &in) {
    for (auto &entry : in.items()) {
      std::string k = entry.key();
      vimRemaps[k[0]] = entry.value();
    }
  }
 void Provider::loadExtraLanguage(json &entry) {
    if (!entry.is_object())
      return;
    Language language;
  ...
  std::string Provider::getFileToOpen(std::string inp, bool reverse) {
    std::string path = inp;
    fs::path p(path);
    std::string partial;
    if (p.has_filename()) {
  ...
  fs::path *Provider::getHomeFolder() {
#ifdef _WIN32
