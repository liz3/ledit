...
State::State(float w, float h, int fontSize) {
...
void State::invalidateCache() { cacheValid = false; }
...
void State::freeVim() {
  ...
  void State::activateLastCommandBuffer() {
    if (isCommandRunning)
      return;
  ...
  void State::tryCopyInput(Utf8String &in) {
  ...
  void State::saveNew() {
    ...
    void State::switchLineHighlightMode() {
    ...
    void State::open(bool inNewWindow) {
    if (mode != 0)
      return;
...
bool State::execCommand(std::string command) {
  ...
  void State::runCommand(std::string cmd) {
    if (!cmd.size())
      return;
  ...
  const Language *State::try_load_language(const std::string &name,
  ...
  void State::directlyEnableLanguage(std::string name) {
    const Language *lang =
  ...
  void State::inform(bool success, bool shift_pressed, bool ctrl_pressed) {
  ...
  const std::vector<const Language *> State::getAllLanguages() {
...
void State::fold() { status = cursor->fold(); }
void State::renderCoords() {
  ...
  void State::deleteActive() { deleteCursor(activeIndex); }
  ...
  void State::addCursorWithExisting(std::string path) {
