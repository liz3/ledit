...
FontAtlas::FontAtlas(std::string path, uint32_t fontSize) {
  errors.clear();
...
RenderChar FontAtlas::render(char32_t c, float x, float y, Vec4f color) {
  if (c >= 128 || c < 32)
    lazyLoad(c);
...
float FontAtlas::getColonWidth() { return virtual_fs * scale; }
void FontAtlas::ensureTab() {
  if (entries.count(U'\t'))
    return;
...
float FontAtlas::getAdvance(char32_t c) {
  if (c >= 128 || c < 32)
    lazyLoad(c);
  return entries[c].advance * scale;
}
size_t FontAtlas::fontSelectSize(uint32_t size, FT_Face face) {
  size_t i;
...
void FontAtlas::readFont(std::string path, uint32_t fontSize,
                         bool shouldRender) {
...
void FontAtlas::resizeFonts(uint32_t size) {
  for (auto &face : faces) {
    if (face->hasColor)
...
void FontAtlas::changeScale(int diff) {
...
void FontAtlas::renderFont(uint32_t fontSize, FontFace *faceEntry) {
  if (wasGenerated) {
...
void FontAtlas::lazyLoad(char32_t c) {
  if (entries.count(c))
    return;
  if (c == U'\t') {
    ensureTab();
  ...
  (&entry)->data = new uint8_t[(int)entry.width * (int)entry.height * 4];
  if (entry.hasColor) {
    for (size_t i = 0; i < ((int)entry.width * (int)entry.height) * 4; i += 4) {
...
float FontAtlas::getAdvance(Utf8String line) {
  float v = 0;
...
float FontAtlas::getAdvance(std::string line) {
  float v = 0;
...
std::vector<float> *FontAtlas::getAllAdvance(Utf8String &line, int y) {
...
bool FontAtlas::isColorEmojiFont(FT_Face &face) { return FT_HAS_COLOR(face); }